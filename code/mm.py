...
# 假设已经定义了VNet, LAHeart, RandomRotFlip, RandomCrop, ToTensor, TwoStreamBatchSampler, losses等
...
def compute_influence(self, grad_vec_wrt_val_loss, x, y, w_match):
    inv_H = self.compute_inverse_hessian(x, y, w_match)
    grads_train_per_ex = self.compute_grads_train_per_ex(x, y, w_match)
    ...
    return influences
#计算逆Hessian矩阵
def compute_inverse_hessian(self, x, y, w_match):
    self.model.train()
    hessian = None
    ...
    return inv_hessian
#计算每个训练样本的梯度
def compute_grads_train_per_ex(self, x, y, w_match):
    self.model.train()
    grads = []
    for i in range(x.size(0)):
        self.optimizer.zero_grad()
...
def train_step(self, train_loader, test_loader, iter_num, ep):
    for i_batch, sampled_batch in enumerate(train_loader):
        ...
        if ep > self.inf_warm and iter_num % (self.inner_steps * self.args.batch_size) == 0:
            ...
            influences = self.compute_influence(grad_vec_wrt_val_loss, volume_batch, label_batch, self.weights)
